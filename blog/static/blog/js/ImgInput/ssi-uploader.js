...
//----------------------------UPLOADFILES------------------------------------
    ...
    var finishUpload = function (thisS) {//when every uplaod ends
        ...
        thisS.inProgress = 0;
        if (files.length!=0){
            $("button#insert").removeAttr("disabled").css("background", "rgb(103, 181, 255)").addClass("buttoninsert");
        }
    };
    ...
    var locale = {
        ...
        zh: {
            ...
            clear: '清除',
            drag: '拖放文件以上传',
            sizeError: '$1 超出 $2 最大限制',// $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
